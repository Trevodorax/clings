...
kata_list_fetch_result_t get_kata_list(void) {
    ...
    const int8_t kata_list_size = sizeof(KATA_NAME_LIST) / sizeof(const char *);
    for (int i = 0; i < kata_list_size; i++) {
        sized_string_t name = new_sized_string_from_string((char *) KATA_NAME_LIST[i]);
...
sized_string_t kata_path_from_name(sized_string_t kata_name) {
    sized_string_t base_path = new_sized_string_from_string(KATAS_BASE_PATH);
...
sized_string_t c_file_name_of(sized_string_t kata_name) {
    if(kata_name.len == 0) {
        return new_sized_string_from_string("_.c");
    }
    sized_string_t cleaned_file_name = clean_string_to_file_name(kata_name);
    sized_string_t extension = new_sized_string_from_string(".c");
