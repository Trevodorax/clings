...
run_kata_result_t run(run_kata_result_t compilation);
...
run_kata_result_t run_kata(const kata_t kata) {
    ...
    run_kata_result_t compiled = compile(kata);
    run_kata_result_t execution = run(compiled);
...
run_kata_result_t compile(const kata_t kata) {
    ...
    if (compilation_process == NULL) {
        return open_process_failed(kata, compile_command);
    }
...
run_kata_result_t run(const run_kata_result_t compilation) {
    ...
    if (execution_process == NULL) {
        return open_process_failed(compilation.kata, new_sized_string_from(RUN_COMMAND));
    }
